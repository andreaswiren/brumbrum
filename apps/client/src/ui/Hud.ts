...
export class Hud {
  ...
  constructor(
    ...
    document.querySelector('#ui')!.innerHTML = `
      ...
      <div id="controller-status" class="controller-status">GAMEPAD READY · PRESS A CONTROLLER BUTTON</div><section id="help-panel" hidden><button id="close-help" aria-label="Close controls">×</button><div class="eyebrow">GET OUT THERE</div><h2>Make your own line.</h2><p>Accelerate toward the yellow flags. The first jump is straight ahead.</p><dl><dt>W</dt><dd>Throttle</dd><dt>S</dt><dd>Brake / hold at stop to reverse</dd><dt>A / D</dt><dd>Steer + lean / air tilt</dd><dt>Space</dt><dd>Hold to preload, release to jump</dd><dt>↑ / ↓</dt><dd>Weight shift / pitch / wheelie</dd><dt>← / →</dt><dd>Extra lean / air roll</dd><dt>Shift</dt><dd>Rear brake / slide</dd><dt>R / Home</dt><dd>Safe reset / return to start</dd><dt>C / F3</dt><dd>Cycle camera / diagnostics</dd></dl><p>Hold ↓ with throttle for a wheelie. Use Space or gamepad A to charge preload; release to jump. Release and reapply A/D or the left stick after takeoff to tilt sideways. Held steering stays stable over bumps. B: bail out.<br>Hold S / LT at a stop with throttle released to reverse. The bike is pushed backward with one foot; other vehicles use reverse gear.<br>Xbox: RT throttle · LT brake / reverse · left stick steer/weight · right stick camera orbit · A hold/release preload (except truck) · D-pad left/right lean · LB rear brake · Y camera · R3 reset · Menu help. Press a gamepad button to connect. Click once for audio if the browser requires it.</p></section>
  ...
  update(dt: number, bike: Motorcycle, world: TerrainWorld, engine: AbstractEngine): void {
    ...
    document.querySelector('#gear')!.textContent = bike.reversing
      ? bike.kind === 'bike'
        ? 'PUSH'
        : 'R'
      : bike.speed < 0.8
        ? 'N'
        : String(Math.min(5, Math.floor(bike.speed / 8) + 1));
